...
func DefaultLexers() []Lexer {
	return []Lexer{
		...
		NewSeriesLexer(
			// s02, S01E01
			// `(?i)^s(?P<s>[0-8]?\d)[\-\._ ]?(?:e(?P<e>\d{1,5}))?\b`, // candi 1
			// s02, S01E01, S01E23E24
			`(?i)^s(?P<s>[0-8]?\d)[\-\._ ]?(?:e(?P<e>\d{1,5}(?:[Ee]\d{1,5})*))?\b`, // candi 1 replacer
			// S01S02S03
			`(?i)^(?P<S>(?:s[0-8]?\d){2,4})\b`,
			// 2x1, 1x01
			`(?i)^(?P<s>[0-8]?\d)x(?P<e>\d{1,3})\b`,
			// S01 - 02v3, S07-06, s03-5v.9
			`(?i)^s(?P<s>[0-8]?\d)[\-\._ ]{1,3}(?P<e>\d{1,5})(?:[\-\._ ]{1,3}(?P<v>v\d+(?:\.\d+){0,2}))?\b`, // candi 2
			// Season.01.Episode.02, Series.01.Ep.02, Series.01, Season.01
			`(?i)^(?:series|season|s)[\-\._ ]?(?P<s>[0-8]?\d)(?:[\-\._ ]?(?:episode|ep)(?P<e>\d{1,5}))?\b`, // candi 3
			// Vol.1.No.2, vol1no2
